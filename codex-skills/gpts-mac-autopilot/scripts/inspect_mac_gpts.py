...
def main():
    ...
    for relative_path in IMPORTANT_FILES:
        ...
        files[relative_path] = {"exists": path.exists(), "path": str(path)}

    knowledge_files = sorted(str(path) for path in knowledge_root.rglob("*.md")) if knowledge_root.exists() else []
    ...
    output = {
        ...
        "env": {
            ...
            "workspace_root_looks_macos": env_values.get("WORKSPACE_ROOTS", "").startswith("/"),
        },
        "openapi": {
        ...
        "commands": {
            "python_3_13": run_command(["python3.13", "--version"]),
    ...
    if sys.platform != "darwin":
        output["warnings"].append("This inspection is running on a non-macOS host, so command results describe the current host, not a target Mac.")
    if output["env"]["first_workspace_root"] and not output["env"]["workspace_root_looks_macos"]:
        output["warnings"].append("WORKSPACE_ROOTS does not start with a macOS-style path.")
