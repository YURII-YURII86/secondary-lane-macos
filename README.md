...
Или просто дважды кликни по `Запустить Second Lane.command`.
...
Подробная пошаговая инструкция для первого запуска:
...
Эти команды используют repo-local `.venv`, не требуют ручной пляски с глобальным окружением и дают быстрый способ подтвердить, что проект реально живой.

Можно передать свои target-файлы:
...
tests/                   # smoke и regression tests
...
Для подробной установки macOS-версии есть два отдельных входа:

- `docs/MAC_FIRST_START.md` — подробная ручная инструкция для первого запуска;
- `codex-skills/gpts-mac-autopilot/` — skill для Codex, чтобы почти автоматически развернуть проект на Mac.
...
Or double-click `Запустить Second Lane.command`.
...
Step-by-step first-start guide:
...
These commands use the repo-local `.venv`, avoid global-environment drift, and give you a fast way to confirm the project is actually working.

You can also pass explicit targets:
...
### macOS Setup Materials

For the detailed macOS install flow there are two separate entry points:

- `docs/MAC_FIRST_START.md` — the detailed manual first-start guide;
- `codex-skills/gpts-mac-autopilot/` — the Codex skill for near-automatic Mac deployment.
